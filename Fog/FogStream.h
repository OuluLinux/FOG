...
public:
	friend FogStream& dummy_blank_line(FogStream& s) {
		return s.dummy_blank_line();
	}
	
	friend FogStream& next(FogStream& s) {
		return s.next();
	}
	
	friend FogStream& start(FogStream& s) {
		return s.start();
	}
...
public:
	...
	~FogStreamIndent() {
		_stream.set_depth(_depth, FogStream::COLUMNS);
	}
};
